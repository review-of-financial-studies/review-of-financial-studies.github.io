...
::: {.callout-important}
The DE will enforce the [template README](https://social-science-data-editors.github.io/template_README/) because consistent adherence dramatically speeds up the review process.
:::

## Review prompt

Unfold the prompt below and copy it to use with the LLM of your choice for detailed feedback on your README. Be aware that LLMs like Claude or ChatGPT will rarely tell you your README is fine as-is. They tend to suggest changes whether or not changes are needed. Use your judgment to decide which suggestions are worth acting on.

```{=html}
<div class="prompt-section">
  <details class="prompt-details">
    <summary class="prompt-summary">
      <span class="prompt-summary-arrow">▶</span>
      <span class="prompt-summary-label">View README review prompt</span>
    </summary>
    <div class="prompt-content">
      <div class="prompt-actions">
        <button class="copy-prompt-btn" id="copyPromptBtn" onclick="copyPrompt()">Copy to clipboard</button>
      </div>
      <pre id="promptText" class="prompt-text">Loading prompt…</pre>
    </div>
  </details>
</div>

<style>
  .prompt-section { margin: 1rem 0 2rem 0; }
  .prompt-details {
  ...
  .prompt-summary-arrow {
    display: inline-block;
    transition: transform 0.15s ease;
    font-size: 0.75rem;
  }
  .prompt-details[open] .prompt-summary-arrow { transform: rotate(90deg); }
  .prompt-content {
  ...
  .copy-prompt-btn:hover { background: #0860c7; }
  .copy-prompt-btn.copied { background: #1a7f37; }
  .prompt-text {
    background: #f6f8fa;
    border: 1px solid #d0d7de;
    border-radius: 6px;
...
<script>
  (function loadPrompt() {
    fetch('assets/prompts/readme-review.txt')
      .then(function (r) {
      ...
      .catch(function () {
        document.getElementById('promptText').textContent =
          'Could not load the prompt file. See assets/prompts/readme-review.txt in the repository.';
      });
  })();
...
All data are publicly available and downloaded automatically by the replication code.`
        ...
        {
          text: "Do you provide a statement about rights?",
          label: "My README includes a rights statement.",
...
I certify that the author(s) of the manuscript have documented permission to redistribute/publish the data contained within this replication package.`
        ...
        {
          text: "Do you have the legal right to use and redistribute the data?",
          label: "I confirm legal rights for data use and redistribution.",
          example: `The authors have legitimate access to all data used in this manuscript.
      ...
      questions: [
        {
          ...
          example: `The code was last run using Stata 18 MP. The required user-written packages are estout and reghdfe. All packages are included in the ado directory.`
        },
        {
...
Approximate time needed to reproduce the analyses on a scientific computing cluster with 200 cores: 4 days. In particular, programs/monte_carlo.py takes 90% of the computing time.`
        ...
        {
          text: "How much disk space is required in total?",
          label: "My README includes estimated storage space requirements.",
          example: `Approximate storage space needed: 25 MB - 250 MB`
        },
        {
          text: "On what type of machine was the code last run?",
          label: "My README includes information about the computing environment.",
