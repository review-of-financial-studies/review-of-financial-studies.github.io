...
- **Review**: Authors must prepare a replication package that complies with the CDSP and submit it via the [RFS Dataverse](https://dataverse.harvard.edu/dataverse/rfs). The package should be prepared for eventual public release (e.g., using pseudo or synthetic data, instructions for replicators), but will remain private during the review phase. Any issues encountered (e.g., execution errors or output discrepancies) will be resolved directly between the DE and the authors. The review process typically involves the following steps:
  ...
  3. Once the code runs successfully, the DE shares the environment with the authors, who then upload all required raw data that cannot be shared publicly to the environment.
  4. The DE verifies that the outputs reproduce the results reported in the submission.
  5. After verification is complete, the DE releases the replication package on the RFS Dataverse and permanently and irrevocably deletes the virtual environment and any shared data.
