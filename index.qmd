...
- **Review**: Authors must prepare a replication package that complies with the CDSP and submit it via the [RFS Dataverse](https://dataverse.harvard.edu/dataverse/rfs). The package should be prepared for eventual public release and, therefore, must include pseudo or synthetic data in place of any non-public data. To help authors prepare a comprehensive README, the DE provide a [README Checker](readme.qmd) that walks through all required elements with best-practice examples. During the review phase, the package will remain private. Any issues encountered (e.g., execution errors or output discrepancies) will be resolved directly between the DE and the authors. The review process typically involves the following steps:
...
- The DE maintain a [README Checker](readme.qmd) that walks through all required elements with best-practice examples
