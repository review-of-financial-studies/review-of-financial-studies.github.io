...
- **Review**: Authors must prepare a replication package that complies with the CDSP and submit it via the RFS Dataverse￼. The package should be prepared for eventual public release (e.g., using pseudo or synthetic data, instructions for replicators), but will remain private during the review phase. Any issues encountered (e.g., execution errors or output discrepancies) will be resolved directly between the DE and the authors. The review process typically involves the following steps:
    1. The DE configures a secure, dedicated virtual environment provided by the RFS, based on the instructions in the [README](https://social-science-data-editors.github.io/template_README/).
    2. The DE verifies that the code executes successfully in this environment using the materials initially provided.
    3. Once the code runs successfully, authors supply all required original data to the environment.
    4. The DE verifies that the outputs reproduce the results reported in the submission.
    5. After verification is complete, the DE permanently and irrevocably deletes the virtual environment.
...
Most common pitfalls can be avoided by ensuring that:

- The replication package includes a [README](https://social-science-data-editors.github.io/template_README/) that follows the Social Science Data Editors template (version 1.1) for standardized documentation.
- A [LICENSE file](license.qmd) is included and clearly specifies the terms governing the use of the code and data.

In addition, the following topics frequently arise during the review process:
